...
predictionsPath = '../../task2/testing/predictions.csv';
...
filteredTestSet5 = testSet(valve_faults_indices, :);

disp(['Number of valve faults cases: ', num2str(height(filteredTestSet5))]);


%% Creating the table for predictions

% Let's create a table initially with all NaN values
predictionsTable5 = table(NaN(numel(cases), 1), 'VariableNames', {'Predictions'});

% We assign 100 to normal elements, unknown anomalies and bubble contamination (i.e. those not present in filteredTestSet)
% that indicate the normal opening ratio of soleniode valves
predictionsTable5.Predictions(normal_indices) = 100;
predictionsTable5.Predictions(unknown_anomalies_indices) = 100;
predictionsTable5.Predictions(bubbles_indices) = 100;