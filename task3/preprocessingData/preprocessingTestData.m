...
% Create a table with only the bubble contamination cases cases, keeping the ID
filteredTestSet = testSet(bubbles_indices, :);

disp(['Number of bubbles contamination cases: ', num2str(height(filteredTestSet))]);
...
% We assign 0 to normal elements, unknown anomalies and valves faults (i.e. those not present in filteredTestSet)
predictionsTable.Predictions(normal_indices) = 0;
predictionsTable.Predictions(unknown_anomalies_indices) = 0;
predictionsTable.Predictions(valve_faults_indices) = 0;