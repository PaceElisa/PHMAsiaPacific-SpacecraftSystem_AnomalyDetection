...
%% Filter Only Valve Fault Anomalies
...
% Create a table with only the valve fault cases, keeping the ID
filteredTestSet = testSet(valve_faults_indices, :);

disp(['Number of valve fault cases: ', num2str(height(filteredTestSet))]);
...
% We assign 0 to normal elements, unknown anomalies and bubble anomalies (i.e. those not present in filteredTestSet)
